...
int main(int argc, char* argv[]) {
    ...
    // Opens the file containing the list of SS servers avaiable
	servers = fopen("servers","r");
	if (servers == NULL)exit(1);

    // Scans the total number of servers
    fscanf(servers, "Total Servers: %d", &nSSservers);
	fileList = (char**)malloc(sizeof(char*)*30);
	if (fileList == NULL) exit(1);
	
    // Creates two arrays of pointers to pointers
    // The first for the IPs of each server
	IPSS = (char**)malloc(sizeof(char*)*nSSservers);
	if (IPSS == NULL) exit(1);
    // And the second for the ports of each server
	ports = (char**)malloc(sizeof(char*)*nSSservers);	
	if (ports == NULL) exit(1);
    
    // Fills both the arrays from the above with the valid information
	i = 0;
	while (i < nSSservers){
    ...
    // Tries to open the file containing the name of all the files avaiable in each SS server
	files = fopen("files","r");
    // If it doesn't exist, starts filling it with the fields for further uploads
	if (files == NULL) {
		files = fopen("files","w");
    ...
    // Extracts from the file variables for further use
	fseek(files, 0, SEEK_SET);
    ...
    // Checks if the user wants to use other port than the standart one
	if (argc == 3) {
		if (!strcmp(argv[1], "-p")) {
    ...
    // Creates two threads, one for each protocol
    // They'll both be operating the CS server
...
// void saveinFile();
//  If the server receives another file, it updates the file containing the list of files
void saveinFile() {
	int i = 0;
	files = fopen("files","w");
	fseek(files, 0, SEEK_SET);
...
// int search_file_list(char ** List, int nf, char name[]);
//  Searchs in the file that contains the list of files a specific file with the same name as the argument char name[]
int search_file_list(char ** List, int nf, char name[]) {
	int i = 0;
	if (nf == 0) {
		return 0;
...
// int sendFilesSS(char* file, int size, char** ip, char** portss);
//  Sends for each SS server the file that the client wants to upload
int sendFilesSS(char* file, int size, char** ip, char** portss) {
...
// void *UDPserver(void *arg);
//  The thread that runs this function keeps on reading the message it received.
//  Then, it compares it with all the valid options the user can send and acts specifically depending on the option.
//  After that, sends to the user a response containing the status of the operation he requested.
void *UDPserver(void *arg) {
	int i;
